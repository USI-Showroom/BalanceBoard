...
void setup() {
  ...
  try {
    ...
    println("Serial port error. Try changing PORT_NUM");
    exit();
  }
  calSamples = 0;
  calDone = false;
...
void draw() {
  ...
  if (currentState.equals("CALIBRATION")) {
    ...
    textAlign(CENTER);
    fill(fontColor);
    text(String.format("Time left for calibration: %d", secondsToGo), 0, 0);
    popMatrix();
    ...
    fill(fontColor);
    textAlign(CENTER);
    text("Attempt finished, please step down from the board", 0, 0);
    popMatrix();
    ...
    textAlign(CENTER);
    translate(width / 2, height / 2);
    text("Step on the board to begin!", 0, 0);
    popMatrix();  
...
//    popMatrix();
    ...
    text(String.format("Time left: %d", secondsToGo), 0, 0);
    popMatrix();
    //the circle
    pushMatrix();
    ...
    fill(fontColor);
    text(String.format("Score: %.1f / %d", score, numberOfCircles), 0, 0);
    popMatrix();
  }
}
...
void showScore() {
    ...
    if (bestScore > 0) {
      ...
      textAlign(LEFT);
      translate(20, 60);
      text(String.format("Best score: %.2f / %d", bestScore, numberOfCircles), 0, 0);
      popMatrix();
    }
    if (lastScore > 0) {
      pushMatrix();
      ...
      textAlign(LEFT);
      translate(20, 90);
      text(String.format("Your score: %.2f / %d", lastScore, numberOfCircles), 0, 0);
      popMatrix();
    }   
}
